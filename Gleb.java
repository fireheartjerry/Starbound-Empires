...
public class Gleb
{
    ...
    */
    ...
    {
	playSound ("Assets/bg6.wav");
    }


    public static void playSound (String soundPath)
    {
	final String finalPath = soundPath;
	Thread musicThread = new Thread (new Runnable ()
	{
		...
		try
		{
		    ...
		    if (!audioFile.exists ())
		    {
			System.err.println ("Audio file not found: " + finalPath);
			return;
		    }
	...
	new Thread (new Runnable ()
	{
	    public void run ()
	    {
		playSound ("Assets/menuswitch.wav");
	    }
	}
	).start ();
    }
	...
	{
	    c.clearRect (112, 593, 40 + 10 * ((int) Math.log (seconds_past)), 23);
	}
	...
	if (c.isCharAvail ())
	{
		...
		// either you are constrained by the total amount of people available to switch, i.e peopleCounter[switchFrom]
		// or you are constrained by the amount of energy you have, i.e resourceCounter[1] / switchCost
		//                                                                amount of energy / cost of switching one person
		// also, the cost of switching increases each time you switch by 5%

		numSwitch = inputNumber (0, min (peopleCounter [switchFrom], (int) (resourceCounter [1] / switchCost)));
		switchCost *= 1.05; // increase by 5% the cost
	...
	if (c.isCharAvail ())
	{
	    ...
	    {
		currentRegion++; // progress to the next planet

		// mostly, regular costs will be subtracted when you conquer a planet
		// these will be reservesNeeded[currentPlanet] and soldiersNeeded[currentPlanet]
		// but there are random events that can happen as well
		if (Math.random () < 0.1)
		{
		    // 10% chance for the alien forces to be caught off guard
		    double reductionFactor = 0.9 - Math.random () * 0.4;
		    c.println ("The alien forces were caught off guard by your powerful military strategy. All costs are reduced.");
		...
		else if (Math.random () < 0.1)
		{
		    ...
		    c.println ("The alien forces were underestimated and a bloody battle ensued. All costs are increased.");
		...
		// if the player is unlucky enough that the costs are increased beyond what they have, they will die
		// thus, all resources are set to 0, amount unemployed and soldiers are set to 0, and number of workers and doctors are decreased by 40-80%
		if (resourceCounter [0] < 0 || peopleCounter [2] < 0)
		{
		    ...
		    c.println ("You did not have enough soldiers or resources and you suffered a crushing defeat. The aliens looted all of your resources.");
		    currentRegion--;
		}
		else
		{
